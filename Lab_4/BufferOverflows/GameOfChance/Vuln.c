...
void make_dir() {
    ...
    if (stat(DATADIR, &st) == -1) {
        if (mkdir(DATADIR, 0700)) {
            fatal("Failed to create the data directory");
        }
    }
}
...
int main() {
    ...
    char token_id[30] = {0};
    printf("Provide me your token id. This is the path where your data file will be stored, so you should make it unguessable by other players. (e.g.: sup3rs3crett0k3n)\n");
    printf("Token id: ");
    scanf("%26s", token_id);

    sprintf(DATAFILE, "%s/%s", DATADIR, token_id);
...
void register_new_player() {
    ...
    if (fd == -1)
        fatal("in register_new_player() while opening a file.");
